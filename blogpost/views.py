...
class IndexView(ListView):
    model = Post
    # Post.objects.all().filter(category__navigation_id__range=)
...
class NavigationView(CategoryView):
    ...
    def get_queryset(self):
        navigation = get_object_or_404(Navigation, pk=self.kwargs.get('pk'))

        cates = navigation.category_set.all()
        query_set = cates[0].post_set.all()
        for i in range(1, len(cates)):
            query_set = query_set | cates[i].post_set.all()
